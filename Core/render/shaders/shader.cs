...
namespace Core.render.shaders
{
    public sealed class Shader
    {
        ...
        public int Handle => programmId; // Add this property to expose programmId
        ...
        {
            shaderProgrammSource = Parse_Shader(vertShaderPath, fragShaderPath, sourceType);
            if (compile)
                Compile();
        }

        ~Shader() { GL.DeleteProgram(programmId); }
        ...
        public int GetAttribLocation(string attribName) { return GL.GetAttribLocation(programmId, attribName); }

        public void Dispose() { GL.DeleteProgram(programmId); }

        public void Unbind() { GL.UseProgram(0); }

        public int Get_Uniform_Location(string name) => uniforms[name];
