...
namespace Core.Particles {
    public class ColorGradient {
        ...
        public ColorGradient(params (float time, Vector4 color)[] colors) {
            foreach(var (time, color) in colors)
                AddColor(time, color);
        }
        ...
        public List<(float time, Vector4 color)> GetColors() { return _colors; }
    }
}
