...
namespace Core.manager.texture {
    ...
    public static class texture_manager {
        ...
        public static int texture_cursor { get; private set; } = 0;

        public static texture_2d load(string texture_name) {

            int handle  = GL.GenTexture();
            Enum.TryParse(typeof(TextureUnit), $"Texture{texture_cursor}", out var parsed_enum);
            if(parsed_enum == null)
                throw new Exception($"Exceeded maximum texture slots OpenGL can nativly support. Count: [{texture_cursor}]");
